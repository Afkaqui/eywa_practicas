...
interface NavigationSidebarProps {
  currentView: string;
  onNavigate: (view: 'hero' | 'diagnostic' | 'portfolio' | 'mobile' | 'notifications' | 'settings' | 'validator') => void;
  onLogout: () => void;
}

export function NavigationSidebar({ currentView, onNavigate, onLogout }: NavigationSidebarProps) {
  const [isExpanded, setIsExpanded] = useState(true);

  const navItems = [
    { id: 'hero', icon: LayoutDashboard, label: 'Panel Principal', color: 'emerald' },
    { id: 'diagnostic', icon: Stethoscope, label: 'Diagnóstico', color: 'blue' },
    { id: 'validator', icon: CheckCircle, label: 'Validador de Proyectos', color: 'green' },
    { id: 'portfolio', icon: BarChart3, label: 'Portfolio', color: 'purple' },
  ];
            ...
            <div className="mb-3 px-4 py-3 bg-gray-50 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 truncate">admin@eywa.com</div>
                </div>
              </div>
            </div>
          ) : (
